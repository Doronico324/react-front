...
function Cart() {
  ...
  useEffect(() => {
    const fetchCartItems = async () => {
      try {
        ...
        const response = await fetch(`https://django-framework-store.onrender.com/user_cart_items/${userId}`, {
          headers: {
  ...
  useEffect(() => {
    const fetchProduct = async (productId) => {
      try {
        const authToken = localStorage.getItem('token');
        const response = await fetch(`https://django-framework-store.onrender.com/products/${productId}`, {
          headers: {
            Authorization: `Bearer ${authToken}`, // Include the token in the request headers
            'Content-Type': 'application/json',
          },
  ...
  const checkout = async () => {
    try {
      ...
      const response = await axios.post(
        `https://django-framework-store.onrender.com/checkout/`,
        {
  ...
  const clearCart = async (userId) => {
    try {
      ...
      const response = await axios.post(
        `https://django-framework-store.onrender.com/clear_cart/${userId}/`,
        {},
        {
          headers: {
            Authorization: `Bearer ${authToken}`,
          ...
          {cartItems.map((cartItem) => {
            ...
            return (
              <div className="cart-item" key={cartItem.id}>
                {product && (
                  <>
                    <img src={`https://django-framework-store.onrender.com${product.image}`} alt={product.name} className="product-image" />
